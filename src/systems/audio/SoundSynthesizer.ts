...
export class SynthesizedSound {
  ...
  play(): void {
    ...
    // Apply envelope
    this.envelope.trigger(now, this.duration);
...
export class SoundSynthesizer {
  ...
  private createSimpleSound(definition: SoundDefinition, frequency: number, _options: SynthPlayOptions): SynthesizedSound {
    ...
    if (definition.spatialEnabled && _options.position) {
      const pannerNode = this.audioContext.createPanner();
      pannerNode.panningModel = 'HRTF';
      pannerNode.setPosition(_options.position.x, 0, _options.position.y);
  ...
  private createChordSound(definition: SoundDefinition, baseFrequency: number, _options: SynthPlayOptions): SynthesizedSound {
  ...
  private createNoiseSound(definition: SoundDefinition, _options: SynthPlayOptions): SynthesizedSound {
  ...
  private createSweepSound(definition: SoundDefinition, startFrequency: number, _options: SynthPlayOptions): SynthesizedSound {
    const oscillator = this.oscillatorPool.acquire();
    const gainNode = this.audioContext.createGain();
    const envelope = new AudioEnvelope(gainNode, definition.envelope);
