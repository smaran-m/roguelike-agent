...
import { DiceSystem } from '../../../src/systems/dice/DiceSystem';
import { Entity, DamageType } from '../../../src/types';
...
describe('CombatSystem', () => {
  ...
  it('should roll dice correctly with different notations', () => {
    ...
    expect(DiceSystem.rollDice('1d6').total).toBe(4);
    
    // 2d6 should roll 8 (4 + 4)
    expect(DiceSystem.rollDice('2d6').total).toBe(8);
    
    // 1d6+3 should roll 7 (4 + 3)
    expect(DiceSystem.rollDice('1d6+3').total).toBe(7);
    
    // 2d6+2 should roll 10 (4 + 4 + 2)
    expect(DiceSystem.rollDice('2d6+2').total).toBe(10);
  });

  it('should handle invalid dice notation gracefully', () => {
    expect(DiceSystem.rollDice('').total).toBe(1);
    expect(DiceSystem.rollDice('invalid').total).toBe(1);
    expect(DiceSystem.rollDice('d').total).toBe(1);
    expect(DiceSystem.rollDice('1d').total).toBe(1);
  });
